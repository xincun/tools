...
DWORD CIni::GetPathName(LPTSTR lpBuffer, DWORD dwBufSize) const
{
	*lpBuffer = _T('\0');
	...
	if (lpBuffer != NULL)
	{
		_tcsncpy_s(lpBuffer, dwBufSize, m_pszPathName, _tcslen(m_pszPathName));
...
DWORD CIni::GetString(LPCTSTR lpSection, LPCTSTR lpKey, LPTSTR lpBuffer, DWORD dwBufSize, LPCTSTR lpDefault) const
{
	...
	if (lpBuffer != NULL)
	{
		_tcsncpy_s(lpBuffer, dwBufSize, psz, _tcslen(psz));
...
BOOL CIni::AppendString(LPCTSTR lpSection, LPCTSTR lpKey, LPCTSTR lpString) const
{
	...
	int nLen = _tcslen(psz) + _tcslen(lpString) + 1;
	TCHAR* pNewString = new TCHAR[];
	_stprintf_s(pNewString, nLen, _T("%s%s"), psz, lpString);
...
DWORD CIni::__StringSplit(LPCTSTR lpString, LPTSTR lpBuffer, DWORD dwBufSize, LPCTSTR lpDelimiter, BOOL bTrimString)
{
	...
	*lpBuffer = _T('\0');
	...
	{
		_tcsncpy_s(lpBuffer, dwBufSize - 1, lpString, _tcslen(lpString));
	...
	while (lpEnd != NULL)
	{
		...
		{
			dwCopied += COPY_LEN + 1;
			_tcsncpy_s(lpTarget, COPY_LEN, pszSeg, _tcslen(pszSeg));
...
BOOL CALLBACK CIni:: __KeyPairProc(LPCTSTR lpString, LPVOID lpParam)
{
	...
	if (psl->lpTarget != NULL)
	{
		...
		_tcsncpy_s(psl->lpTarget, dwCopyLen, psz, _tcslen(psz));
...
void CIni::__IntToString(int nNumber, LPTSTR lpBuffer, int nBase)
{
	...
	default:
		_stprintf_s(lpBuffer, DEF_PROFILE_NUM_LEN + 1, _T("%d"), nNumber);
		break;
	}	
}
...
void CIni::__UIntToString(UINT nNumber, LPTSTR lpBuffer, int nBase)
{
	...
	case BASE_OCTAL:
		_stprintf_s(lpBuffer, DEF_PROFILE_NUM_LEN + 1, _T("%o"), nNumber);
		break;

	case BASE_HEXADECIMAL:
		_stprintf_s(lpBuffer, DEF_PROFILE_NUM_LEN + 1, _T("%X"), nNumber);
		break;

	default:
		_stprintf_s(lpBuffer, DEF_PROFILE_NUM_LEN + 1, _T("%u"), nNumber);
		break;
	}	
}
...
LPTSTR CIni::__StrDupEx(LPCTSTR lpStart, LPCTSTR lpEnd)
{
	...
	LPTSTR psz = new TCHAR[LEN + 1];
	_tcsncpy_s(psz, LEN, lpStart, _tcslen(lpStart));
	psz[LEN] = _T('\0');
	return psz; // !!! Requires the caller to free this memory !!!
}
