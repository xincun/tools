Dotnet IL Editor (DILE) v0.2.6

This is the v0.2.6 release of DILE that contains both disassembling and debugging related enhancements and bug fixes. Some of the new features include correct parsing of 2.0 format permission sets, disassembling of class events, evaluating expressions with generics (type parameters) and enums. The known bugs are described below, but should you find anything else, please let me know using the project's homepage [1] or the following e-mail address: dile.project@gmail.com.
This version also supports disassembling files with PE32+ headers (64-bit assemblies) and debugging on x64 operating systems (I have tested it on Windows Server 2003 x64).

Debugging has been tested with v1.1 and v2.0 applications but theoretically it should work with v1.0, v3.5 and Mono programs as well.
...
Requirements:
  - Windows ;-)
  - .NET Framework 2.0. The program was compiled and tested with the RTM version (v2.0.50727) which is available from MSDN [2] for free.
  - in some cases, the Visual C++ 8.0 files might be missing, these can be installed either from the VS folder (x86: [7] and x64: [8]) or from the Microsoft website (x86: [9] and x64: [10])
  
Source code:
The source code looks awful. There are no comments and the exception handling is also quite simple. But the code is available in the CVS on SourceForge [1] for the curious ones. ;-)
...
Feedback:
  You can contact me via my blog [3], the project's homepage [1] or the following e-mail address: dile.project@gmail.com.
  
The program uses Weifen Luo's DockPanel Suite [4].
  
Links:
  [1] Project homepage: http://dile.sourceforge.net/
  ...
  [7] %ProgramFiles%\Microsoft Visual Studio 8\SDK\v2.0\BootStrapper\Packages\vcredist_x86\vcredist_x86.exe
  [8] %ProgramFiles%\Microsoft Visual Studio 8\SDK\v2.0\BootStrapper\Packages\vcredist_x64\vcredist_x64.exe
  [9] http://www.microsoft.com/downloads/details.aspx?familyid=200b2fd9-ae1a-4a14-984d-389c36f85647&displaylang=en
  [10] http://www.microsoft.com/downloads/details.aspx?displaylang=en&familyid=eb4ebe2d-33c0-4a47-9dd4-b9a6d7bd44da


Copyright (C) 2007 Petrény Zsolt
